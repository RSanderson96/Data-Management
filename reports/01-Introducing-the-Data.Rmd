...
U1 = U + geom_point (aes (x=Run, y=Gender, colour = "Gender", size = 1))
U2 = U1 + geom_point (aes (x=Run, y= Country, colour = "Country", size = 1))
U3Graph = U2 + geom_point (aes (x=Run, y=Education, colour = "Education", size=1)) + theme(text = element_text(size=20)) + labs( title = "The Percentage of Missing Information for Each Course Run", x = "Course Run", y = "% of Unknowns") + guides(size=FALSE) + theme(legend.text = element_text( size = 20))
U4 = U3Graph+ ylim(0,1)

Scaled = U3Graph + ylim(0.85,0.95)
 
```
```{R ShowUnknown, echo = FALSE, fig.width = 20, fig.height = 10}
grid.arrange(U4, Scaled, nrow = 2) 

```

The comparison of the two graphs shows the importance of scale within their design. The first graph emphasises that all runs of the course are missing demographic data for over 85% of their cohorts. The second grpah emphasises how this varies between cohorts - potentially exaggerating this difference as the variations are visualised to appear to be more extreme, for example run 5 compared to run 6.
...
####*Archetype Survey Responses and Weekly Sentiment survey responses*
